...
if (isset($_POST['cancel_order'])) {
    ...
    if ($reason === '')
        $reason = 'Cancelled due to delay of payment.';
...
<body>
                        ...
                        <tbody>
                            <?php while ($row = $res->fetch_assoc()): ?>
                                <tr class="<?= ($row['order_status'] == 'Cancelled') ? 'table-danger' : '' ?>">
                                    <td><?php if ($row['order_status'] != 'Cancelled'): ?><input type="checkbox"
                                                name="paid_orders[]" value="<?= $row['order_id'] ?>"><?php endif; ?></td>
                                    ...
                                    <td class="cancel-reason-col">
                                     ...
                                     <button type="button" class="btn btn-danger btn-sm w-100" onclick="setCancelReasonAndSubmit('?=$row['order_id']?>')">Cancel</button>-->
                                            <select class="form-select form-select-sm mb-1 reason-input"
                                                id="reason_select_<?= $row['order_id'] ?>">
                                                <option value="">- Select reason -</option>
                                                <option value="Due to stock unavailability.">Due to stock unavailability.
                                                </option>
                                                <option value="Due to delivery issue.">Due to delivery issue.</option>
                                                <option value="Due to shipping issue.">Due to shipping issue.</option>
                                                <option value="Due to delay of your response while delivery.">Due to delay of
                                                    your response while delivery.</option>
                                            </select>
                                            <textarea id="reason_text_<?= $row['order_id'] ?>" rows="2"
                                                class="form-control mb-1 reason-input"
                                                placeholder="Type custom reason..."></textarea>
                                            <button type="button" class="btn btn-danger btn-sm w-100 cancel-btn"
                                                id="cancel_btn_<?= $row['order_id'] ?>" disabled
                                                onclick="setCancelReasonAndSubmit('<?= $row['order_id'] ?>')">Cancel</button>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        ...
                                        $isRefundAllowed = (
                                            ...
                                            in_array(strtolower($row['order_status']), ['placed', 'processing','cancelled'])
                                        );
                                        ?>
                                        <a class="btn btn-sm refund-btn <?= $isRefundAllowed ? '' : 'disabled' ?>"
                                            href="<?= $isRefundAllowed ? 'payment_details.php?order_id=' . $row['order_id'] . '&upi=' . urlencode($row['upi_id'] ?? '') . '&phone=' . urlencode($row['payment_phone'] ?? '') : '#' ?>"
                                            <?= $isRefundAllowed ? '' : 'tabindex="-1" aria-disabled="true"' ?>>
                                            Refund
                                        </a>
                                    </td>
    ...
    <script>
        ...
        function setCancelReasonAndSubmit(orderId) {
            ...
            hiddenId.type = 'hidden'; hiddenId.name = 'order_id'; hiddenId.value = orderId;

            const hiddenReason = document.createElement('input');
            hiddenReason.type = 'hidden'; hiddenReason.name = 'cancelled_reason'; hiddenReason.value = reason;

            const hiddenCancel = document.createElement('input');
            hiddenCancel.type = 'hidden'; hiddenCancel.name = 'cancel_order'; hiddenCancel.value = '1';
        ...
        document.querySelectorAll('.reason-input').forEach(input => {
            input.addEventListener('input', toggleCancelButton);
            input.addEventListener('change', toggleCancelButton);
        });
        ...
        function toggleCancelButton() {
            ...
            const select = row.querySelector('select.reason-input');
            const textarea = row.querySelector('textarea.reason-input');
            const button = row.querySelector('button.cancel-btn');

            const hasReason = (select && select.value.trim() !== '') || (textarea && textarea.value.trim() !== '');
